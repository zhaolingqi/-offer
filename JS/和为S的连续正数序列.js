/*
 * @Author: Zhaolq 
 * @Date: 2019-09-18 14:16:35 
 * @Last Modified by: Zhaolq
 * @Last Modified time: 2019-09-18 15:23:24
 */

 /**
  * 题目描述
        小明很喜欢数学,有一天他在做数学作业时,要求计算出9~16的和,他马上就写出了正确答案是100。
        但是他并不满足于此,他在想究竟有多少种连续的正数序列的和为100(至少包括两个数)。没多久,
        他就得到另一组连续正数和为100的序列:18,19,20,21,22。现在把问题交给你,你能不能也很快的找出所有和为S的连续正数序列? Good Luck!
    输出描述:
        输出所有和为S的连续正数序列。序列内按照从小至大的顺序，序列间按照开始数字从小到大的顺序

        双指针，等差数列求和公式，当和小于sum 尾指针加1，和大于sum 首指针加1
  */
  

// function FindContinuousSequence(sum) {
//     var seq = []
//     var seqSum = 0
//     var seqT = []
//     for(let i = 1; i <= parseInt(sum / 2); i++) {
//         seq = []
//         seqSum = 0
//         for(let j = i; j < sum; j++) {
//             seqSum += j
//             seq.push(j)
//             if(seqSum >= sum) break
//         }
//         if(seqSum === sum)
//             seqT.push(seq)
//     }
//     if(seqT.length === 0) console.log([])
//     seqT.forEach((e) => {console.log(e)})
// }

// FindContinuousSequence(1)
function FindContinuousSequence(sum) {
    var first = 1
    var end = 2
    var seq = []
    while(first < end) {
        if(Sum(first, end) < sum) {
            end++
        } else if(Sum(first,end) > sum) {
            first++
        } else {
            seq.push(Print(first, end))
            first ++
            end ++
        }
    }
    return seq
}

function Sum(first, end) {
    return (((first + end) * (end - first + 1)) / 2)
}

function Print(first, end) {
    var arr = []
    for(let i = first; i <= end; i ++) {
        arr.push(i)
    }
    return arr
}
console.log(FindContinuousSequence(100))